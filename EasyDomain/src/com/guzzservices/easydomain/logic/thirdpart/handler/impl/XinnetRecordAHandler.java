...
public class XinnetRecordAHandler implements IRecordAHandler{
	
	DefaultHttpClient httpclient = new DefaultHttpClient();
	DomainProvider xinProvider ;
    ...
    
	{
        ...
        xinProvider = new DomainProvider() ;
        xinProvider.setKeyword("xinnet") ;
        xinProvider.setProviderName("新网");
        xinProvider.setDescription("http://www.xinnet.com/");
        xinProvider.setMaxARecords(20) ;
    }
...
//Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 5.1; Trident/4.0; CIBA; .NET CLR 1.1.4322; .NET CLR 2.0.50727)
		HttpPost httpost = new HttpPost("http://dcp.xinnet.com/domainLogin.do?method=domainEnter");

		List<NameValuePair> nvps = new ArrayList<NameValuePair>();
		nvps.add(new BasicNameValuePair("domainName", domainName));
...
//		System.out.println("Login form get: " + response.getStatusLine());
		...
		if(content.contains("My DNS功能")){
			
			httpost = new HttpPost("http://www.xinnet.com/domainLogin.do");

			nvps = new ArrayList<NameValuePair>();
			nvps.add(new BasicNameValuePair("domain", domainName));
			nvps.add(new BasicNameValuePair("method", "domainDNS"));
			...
			String startStr = "<input type=\"hidden\" name=\"strUrl\" value=\"" ;
			...
			String url = content.substring(start + startStr.length(), end) ;
	...
	public List<RecordA> parseServerPageContentToRecordAs(String content){
		int start = content.indexOf("<input type=hidden name=recordtype value=\"A\">") ;
		...
		while(start >= 0){
			...
			//domain
			int m_start = content.indexOf("<TD noWrap>", start) ;
			int m_end = content.indexOf("</TD>", m_start) ;
			ra.setVisitedDomain(content.substring(m_start + "<TD noWrap>".length(), m_end)) ;
			
			//IP

			int m_start2 = content.indexOf("<TD noWrap>", m_end) ;
			int m_end2 = content.indexOf("</TD>", m_start2) ;
			ra.setIP(content.substring(m_start2 + "<TD noWrap>".length(), m_end2)) ;
			
			
			//TTL

			int m_start3 = content.indexOf("<TD noWrap>", m_end2) ;
			int m_end3 = content.indexOf("</TD>", m_start3) ;
			ra.setTtl(content.substring(m_start3 + "<TD noWrap>".length(), m_end3)) ;
			
			//ID
			int m_start4 = content.indexOf("href=\"delrecord?RecordID=", m_end3) ;
			m_start4 = m_start4 + "href=\"delrecord?RecordID=".length() ;
			
			int m_end4 = content.indexOf("\"", m_start4) ;
			ra.setId(content.substring(m_start4, m_end4)) ;
			
			as.add(ra) ;
			
			start = content.indexOf("<input type=hidden name=recordtype value=\"A\">", m_end4) ;
		}
    ...
    protected boolean isListContentRightForParser(String content){
        ...
        if(!content.contains("添加新的A记录")) return false ;
...
//		System.out.println("jump to mydns :" + this.mydnsURL);
		...
		String content = HelpUtil.readStream(entity.getContent(), HelpUtil.ENCODE_GBK) ;
		entity.consumeContent();
...
//            System.out.println(content) ;
	...
	public synchronized boolean addRecordA(RecordA record) throws Exception {        
		...
		//增加新域名
		HttpPost httpost = new HttpPost("http://mydns2.xinnet.com/cgi-bin/addrecord");

		List<NameValuePair> nvps = new ArrayList<NameValuePair>();
		nvps.add(new BasicNameValuePair("content0", record.getIP())); //IP
		nvps.add(new BasicNameValuePair("count", String.valueOf(listAllRecordAs().size() + 1)));
		nvps.add(new BasicNameValuePair("name0", record.getNameA())); //domain
		nvps.add(new BasicNameValuePair("recordtype", "A"));
		nvps.add(new BasicNameValuePair("sbt", "ìá"));
		nvps.add(new BasicNameValuePair("ttl0", record.getTtl())); //ttl
...
//            System.out.println(content) ;
    ...
    public synchronized boolean updateRecordA(String oldNameA,RecordA record) throws Exception {
		...
		HttpPost httpost = new HttpPost("http://mydns2.xinnet.com/cgi-bin/editok");

		List<NameValuePair> nvps = new ArrayList<NameValuePair>();
		nvps.add(new BasicNameValuePair("RecordID", r.getId())); //ID
		nvps.add(new BasicNameValuePair("recordtype", "A"));
		nvps.add(new BasicNameValuePair("content", record.getIP()));
		nvps.add(new BasicNameValuePair("ttl", record.getTtl())); //ttl
...
//			更新成功了
	...
	public synchronized boolean removeRecordA(RecordA record) throws Exception {
		...
		HttpGet get = new HttpGet("http://mydns2.xinnet.com/cgi-bin/delrecord?RecordID=" + r.getId()) ;
